...
const crearDatosConCamposCalculados = (datosBase: Venta[]) => {
  return datosBase.map((item) => ({
    ...
    totalComision: calcularComisionTotal(
      item.ventas,
      item.metaProductosVip,
      item.gafaVip,
      item.monturaVip,
...
const VentaPage = () => {
  ...
  useEffect(() => {
    setTotales({
      ...
      totalImporte: Number(
        ventas
          .reduce((acc, venta) => acc + totalImporte(venta.ventas), 0)
          .toFixed(2)
      ),
      totalDescuento: Number(
        ventas.reduce((acc, venta) => acc + venta.totalDescuento, 0).toFixed(2)
      ),
      totalGranTotal: Number(
        ventas.reduce((acc, venta) => acc + venta.montoTotal, 0).toFixed(2)
      ),
        ...
        <Table className="w-[95%] m-auto p-2 rounded-md bg-white shadow-md">
          ...
          <TableFooter>
            <TableRow>
              ...
              <TableCell className="text-right">
                {formatoMoneda(totales.totalImporte)}
              </TableCell>
              <TableCell className="text-right">
                {formatoMoneda(totales.totalDescuento)}
              </TableCell>
              <TableCell className="text-right">
                {formatoMoneda(totales.totalGranTotal)}
              </TableCell>
              <TableCell className="text-right">
                {formatoMoneda(totales.totalComision)}
              </TableCell>
            </TableRow>
          </TableFooter>
        </Table>
