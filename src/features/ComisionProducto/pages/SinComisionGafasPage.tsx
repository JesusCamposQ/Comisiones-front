...
export const SinComisionGafasPage = () => {
  ...
  const [combinacionProducto, setCombinacionProducto] = useState<Datum[] | null>(null);
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => {
    const fetch = async () => {
      setIsLoading(true);
      const data = await obtenerSinComisionProductoGafa();
      setCombinacionProducto(data);
      setIsLoading(false);
    };
    fetch();
  }, [page]);
      ...
      {isLoading ? (
        ...
        <>
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {filteredData.length > 0 && (filteredData.map((combinacion: Datum, index: number) => (
            ...
            {Object.keys(activeFilters).length == 0 && (
              <tfoot>
                <Paginador filtrar={filteredData} page={page} setPage={setPage} />
              </tfoot>
            )}
          </table>
          <Mensaje
